...
**Time:** 2026-06-25T06:52:33Z — 2026-06-25T06:53:51Z (trigger burst); workload process up since 06:38:58Z
...
```mermaid
flowchart LR
  subgraph op["Operator access path (kubectl port-forward)"]
    pf(("SSO admin<br/>aaron.miles-sysdig.com<br/>create portforward → pod"))
  end
  ...
  subgraph container["Container e5b858b1fe23 — pod langchain-ai-586b98fb9b-q669v<br/>image ai-workload/langchain:1.0.2"]
    ...
    recon["id &amp;&amp; hostname<br/>06:52:34 / 06:53:45"]
    envsec["env|grep API_KEY|PASSWORD|SECRET<br/>06:52:35 / 06:53:46"]
    envllm["env|grep OPENAI|ANTHROPIC (EXFIL markers)<br/>06:52:36 / 06:53:47"]
    findaws["find */.aws/credentials<br/>06:52:37 / 06:53:48"]
    cataws["cat /root/.aws/credentials<br/>06:53:50"]
  end

  subgraph host["EC2 host i-038244f3e51c4ade3"]
    imds[("IMDS 169.254.169.254<br/>/latest/meta-data/iam/security-credentials/<br/>06:52:38 / 06:53:51")]
  end

  pf -. "06:52:33 / 06:53:44 port-forward" .-> entry
...
|---|---|---|
| `06:38:58` | container `e5b858b1fe23` | `python3.11 app.py` starts (root) — the LangChain app, later the parent of all shells |
| `06:52:33` | api-server `ai-workload` | `K8s Portforwarding Detected` — SSO admin `aaron.miles-sysdig.com` port-forwards to pod `langchain-ai-586b98fb9b-q669v` |
| `06:52:34` | container | `id && hostname` — identity/host recon (T1033/T1082) |
| `06:52:35` | container | `env | grep -iE "API_KEY|PASSWORD|SECRET"` — env secret harvest (T1552.001) |
| `06:52:36` | container | `env | grep -iE "OPENAI|ANTHROPIC"` with `EXFIL` markers — LLM key collection |
| `06:52:37` | container | `find … -path "*/.aws/credentials"` — AWS cred discovery (T1552.001) |
| `06:52:38` | EC2 host | `curl http://169.254.169.254/latest/meta-data/iam/security-credentials/` — IMDS theft (T1552.005) |
| `06:53:44` | api-server `ai-workload` | `K8s Portforwarding Detected` — second port-forward to the same pod |
| `06:53:45–47` | container | recon + env-secret + LLM-key sweep repeated |
| `06:53:48` | container | `find … "*/.aws/credentials"` repeated |
| `06:53:50` | container | `cat /root/.aws/credentials` — static AWS key read (T1552.001) |
| `06:53:51` | EC2 host | `curl …/iam/security-credentials/` — IMDS theft repeated (T1552.005) |
...
- **Prior events on this resource (last 7d):** Recurring — Threats Engine shows a matching `Exploitation for Privilege Escalation` group on `langchain-ai` (group `019ef3a0`, "Suspicious Python Script Execution and Privilege Escalation", 2026-06-23) with the same Base64 + Find-AWS-Credentials + Privileged-Shell rule set. This is a repeating pattern on the same workload, not a one-off.
...
- **Rotate exposed credentials now.** Any IAM role exposed via IMDS on `i-038244f3e51c4ade3`, the keys in `/root/.aws/credentials`, and the `OPENAI`/`ANTHROPIC` keys in the pod environment should be treated as compromised and rotated; review CloudTrail (if/when integrated) for STS/IAM use of the node's instance-role credentials after 06:52Z.
...
- **Constrain the access path:** the two `K8s Portforwarding Detected` events show the pod is reachable via `kubectl port-forward` by an SSO admin immediately before each burst — confirm whether the malicious prompt is being delivered over that forwarded port and restrict who can port-forward into `ai-workload`.
